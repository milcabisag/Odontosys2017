...
import OdontoSysPantallaAuxiliares.ObtenerFacturasPaciente;
 ...
 * @author user
 */
public class NotasCreditos extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        jButtonBuscarPaciente.setMargin(new java.awt.Insets(2, 7, 2, 7));
                                ...
                                .addGroup(layout.createSequentialGroup()
                                    .addGroup(layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING, false)
                                        .addGroup(layout.createSequentialGroup()
                                            ...
                                            .addGap(18, 18, 18)
                                            .addComponent(jTextFieldPaciente, javax.swing.GroupLayout.PREFERRED_SIZE, 187, javax.swing.GroupLayout.PREFERRED_SIZE)
    ...
    private void jButtonAceptarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonAceptarActionPerformed
        ...
        }else if(pacActual == null){
            JOptionPane.showMessageDialog(rootPane, "Ingrese Paciente", "Validez de Campo", WIDTH);
        }else{
                NotaCredito nuevo = new NotaCredito();
                nuevo.setFactura(facActual);
                nuevo.setFecha(new Date());
                nuevo.setPaciente(pacActual);
                nuevo.setTotal(Integer.parseInt(jTextFieldTotal.getText().replace(".", "")));
                nuevo.setUsuario(userActual);
                boolean v = false;
                if(facActual.getTipoFactura().compareTo("Crédito") == 0){    //Si la factura es crédito, afecta su saldo
                    if(nuevo.getFactura().getSaldo() >= nuevo.getTotal()){   //Si el total es menor o igual al saldo de la factura
                        nuevo = NotaControlador.insertarNota(nuevo, sesion);
                        insertarDetalle(nuevo, sesion);
                        v = NotaControlador.actualizarSaldo(nuevo, sesion);          //Actualiza el saldo de la factura crédito
                    }else{
                        JOptionPane.showMessageDialog(rootPane, "Monto no puede ser mayor al saldo de la factura", "Nota Crédito", WIDTH);
                    }
    ...
    private void jButtonBuscarFacturaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jButtonBuscarFacturaActionPerformed
        ObtenerFacturasPaciente.pac = pacActual;
        ObtenerFacturasPaciente jDialog = new ObtenerFacturasPaciente(null, true);
        jDialog.setVisible(true);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(NotasCreditos.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new NotasCreditos().setVisible(true);
            }
        });
    }
    //Variables
    ...
    Factura facActual = null;
    int monto = 0;
    public static Usuario userActual;
    ArrayList<DetalleNota> detalle = new ArrayList();
    ...
    private void inicializarTodo() {
        ...
        if(pacActual != null){          //Si es llamado desde otro Frame que no sea inicio
    ...
    private void insertarDetalle(NotaCredito nuevo, Session sesion) {
        detalle = new ArrayList();
        int f = jTableDescripcion.getRowCount();
        for(int x=0;x<f;x++){
            DetalleNota n = new DetalleNota();
            ...
            n.setNotaCredito(nuevo);
            detalle.add(n);
        }
        boolean v = NotaControlador.insertarDetalleNota(detalle, sesion);
        if(v==false){
